  ...
  render() {
    ...
    function printDocument() {
      ...
      for(let i = 0; i < storedRounds.length; i++){
        let pdfInput = document.getElementById(`round${i}`)
        pdf.fromHTML(pdfInput, 10, 15, options)
        pdf.addPage();
      }
    ...
    // subQuestionArray.map((rounds, index) => {
      
    // })

    let masterPdf = <div></div>

    for(let i=0; i < subQuestionArray.length; i++){
      if(i%2 === 0){
        return <Button onClick={() => printDocument()}>Print Answer Key</Button>;
      } else{
        return <div id={`round${i}`}>{subQuestionArray[i]}</div>
      }
    }
    ...
    return (
      <div>
        ...
        <Button onClick={() => printDocument()}>Print Answer Key</Button>
      </div>
    );
  }
}
